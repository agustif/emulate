...
- `GET /app/hook/deliveries` - list retained delivery attempts with cursor and outcome filtering
- `GET /app/hook/deliveries/:id` - inspect retained request and response metadata
- `POST /app/hook/deliveries/:id/attempts` - redeliver the retained payload
...
Delivery history is Store-backed and includes both HTTP responses and transport failures. Redelivery creates a new attempt ID while preserving the original delivery GUID and payload.
